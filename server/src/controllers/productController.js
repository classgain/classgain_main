...
export async function createProduct(req, res, next) { try { const product = await Product.create(productData(req.body, req.file)); res.status(201).json({ success: true, message: 'Product added.', product }); } catch (e) { next(e); } }
export async function updateProduct(req, res, next) { try { const current = await Product.findById(req.params.id); if (!current) return res.status(404).json({ success: false, message: 'Product not found.' }); Object.assign(current, productData(req.body, req.file, current)); await current.save(); res.json({ success: true, message: 'Product updated.', product: current }); } catch (e) { next(e); } }
