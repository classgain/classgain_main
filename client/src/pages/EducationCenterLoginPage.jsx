...
import {
  ...
  loginEducationPartner,
  registerEducationPartner,
...
export default function EducationCenterLoginPage() {
  ...
  const authHeading = authMode === 'signup' ? 'Create Education Center Account' : 'Education Center Sign In';
  ...
  const handleAuthSubmit = async (event) => {
    ...
    const organizationName = authForm.organizationName.trim();
    const contactPerson = authForm.contactPerson.trim();
    const governmentCode = authForm.governmentCode.trim();
    const officialEmail = authForm.officialEmail.trim().toLowerCase();
    const phone = authForm.phone.trim();
    const password = authForm.password.trim();
    const isSignupMode = authMode === 'signup';

    if (isSignupMode && (!organizationName || !contactPerson || !governmentCode)) {
      setStatus({ type: 'error', message: 'Please complete the education center registration details.' });
      return;
    }

    if (!officialEmail || !password) {
      setStatus({ type: 'error', message: 'Official email and password are required.' });
      return;
    }
    ...
    if (isSignupMode && password !== authForm.confirmPassword.trim()) {
    ...
    try {
      ...
      const responseData = isSignupMode
        ? await registerEducationPartner({
      ...
      setStatus({
        ...
        message: isSignupMode
          ? 'Education center account created successfully.'
          : 'Education center sign in successful.'
      });
    } catch (error) {
      setStatus({
        type: 'error',
      ...
      <div className="education-auth-page">
        ...
        <div className="education-auth-grid">
          <section className="education-auth-intro">
            ...
            <h1>Sign in or create your education center dashboard</h1>
            <p>
              Manage course lists, active and inactive programs, scholarship offers, student applications, image uploads,
              and video uploads from one MongoDB-backed education center portal.
            </p>
            ...
            <div className="education-category-admin-actions">
              {educationCategoryOptions.map((category) => (
                <Link
                  key={category.id}
                  to={`/${category.id === 'primary' ? 'startingeducation' : category.id === 'secondary' ? 'highereducation' : 'additionaleducation'}`}
          ...
          <section className="education-auth-card">
            ...
            <form className="education-auth-form" onSubmit={handleAuthSubmit}>
              ...
              <p className="education-auth-form__text">
                {authMode === 'signup'
                  ? 'Create your education center account and open the dashboard immediately.'
                  : 'Sign in to manage courses, scholarship offers, applications, and media uploads.'}
              </p>
                  ...
                  <div className="portal-form__split">
                    <label className="portal-form__field">
                      ...
                      <input
                        type="text"
                        name="governmentCode"
                        value={authForm.governmentCode}
                        onChange={handleAuthChange}
                        placeholder="EDU-REG-2026"
                      />
                    </label>
              ...
              <div className="education-auth-form__meta">
                <span>Local development uses the same Express API and MongoDB database as the dashboard.</span>
              </div>
