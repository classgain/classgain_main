...
function StudentCounsellingDashboard({ token }) {
  ...
  if (error) return <div className="dashboard-page__status dashboard-page__status--error">{error}</div>;
  return <><div className="student-counselling-summary">{['Pending','In Progress','Resolved','Closed'].map((status)=><article key={status}><strong>{requests.filter((item)=>item.status===status).length}</strong><span>{status}</span></article>)}</div><div className="student-counselling-columns"><div><h3>My counselling requests</h3>{!requests.length && <p>No counselling requests are linked to this student account yet.</p>}{requests.map((item)=><article className="student-counselling-request" key={item._id}><div><h4>{item.subject}</h4><span>{item.category} · {new Date(item.createdAt).toLocaleDateString()}</span></div><b className={`status-${item.status.toLowerCase().replace(' ','-')}`}>{item.status}</b><button type="button" onClick={()=>setSelected(selected?._id===item._id?null:item)}>View response</button>{selected?._id===item._id && <div className="student-counselling-response"><p>{item.description}</p><p><strong>Counsellor reply:</strong> {item.adminReply || 'The counselling team has not replied yet.'}</p>{item.image && <img src={`${baseUrl}${item.image}`} alt="Counselling attachment" onClick={()=>window.open(`${baseUrl}${item.image}`,'_blank')}/>}</div>}</article>)}</div><div><h3>Notifications</h3>{!notifications.length && <p>No counselling notifications.</p>}{notifications.map((item)=><button type="button" className={`student-notification ${item.isRead?'':'student-notification--unread'}`} key={item._id} onClick={()=>read(item)}><strong>{item.title}</strong><span>{item.message}</span><small>{new Date(item.createdAt).toLocaleString()}</small></button>)}</div></div></>;
}
...
export default function LoginPage() {
  ...
  const [status, setStatus] = useState(() => ({
    type: 'info',
    message: location.state?.message || (readStoredStudentSession()?.token ? 'Restoring your student session...' : '')
  }));
  ...
  useEffect(() => {
    if (!isDashboardLoading && location.state?.panel) {
      const timer = setTimeout(() => sectionRefs[location.state.panel]?.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
      return () => clearTimeout(timer);
    }
    return undefined;
  }, [isDashboardLoading, location.state]);
            ...
            <div className="student-application-list">
              {resolvedStudent.applicationStatuses.map((application) => (
                <article key={application.id} className="student-application-card">
                  ...
                  <div className="student-application-reply">
                    <b>Education center reply</b>
                    <p>{application.reply || 'No reply yet. The education center will update you here.'}</p>
                  </div>
                </article>
              ))}
            </div>
