...
export async function createEducationApplication(itemId, payload, token) {
...
export function createOrder(payload, token) { return apiRequest('/orders', { method: 'POST', headers: buildStudentHeaders(token), body: JSON.stringify(payload) }); }
export function fetchMyOrders(token) { return apiRequest('/orders/my', { headers: buildStudentHeaders(token) }); }
...
export function fetchMyCounselling(token) { return apiRequest('/counselling', { headers: buildStudentHeaders(token) }); }
export function fetchNotifications(token) { return apiRequest('/notifications', { headers: buildStudentHeaders(token) }); }
export function markNotificationRead(id, token) { return apiRequest(`/notifications/${encodeURIComponent(id)}/read`, { method: 'PATCH', headers: buildStudentHeaders(token), body: JSON.stringify({}) }); }
...
export function fetchProductsByCategory(category, params = {}) {
  const query = buildQueryString(params);
  return apiRequest(`/products/category/${encodeURIComponent(category)}${query}`, { method: 'GET' });
}
